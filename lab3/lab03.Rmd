...
```
html <- readLines("https://github.com/SergeyMirvoda/MD-DA-2018/blob/master/data/forbes.htm") 
...
length(grep("$72 B", html.tests, fixed = TRUE)) == 1 

length(grep("$41 B", html.tests, fixed = TRUE)) > 0 


#как минимум два значения, встречающихся несколько раз. 
length(grep("$36 B", html.tests, fixed = TRUE)) > 1 

length(grep("$5 B", html.tests, fixed = TRUE)) > 1 
...
billion <- 10**9 
html.tests.modification <- gsub(",", "\\.", html.tests) 
worths <- as.double(regmatches(html.tests.modification, regexpr("[0-9.]+", html.tests.modification))) * billion 
